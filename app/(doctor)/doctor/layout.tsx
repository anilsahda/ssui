...
export default function DoctorLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="d-flex flex-column min-vh-100 bg-light" style={{ fontFamily: "Inter, sans-serif" }}>
      {/* HEADER */}
      <header className="d-flex justify-content-between align-items-center px-3 px-md-4 py-2 shadow-sm bg-white border-bottom">
        <h1 className="m-0 text-primary fs-5 fw-semibold">SS App</h1>
        ...
        <div className="d-flex align-items-center gap-2 gap-md-3">
          <button className="btn btn-light rounded-circle shadow-sm p-2"><FiMoon size={18} /></button>
          <button className="btn btn-light rounded-circle shadow-sm p-2"><MdHelpOutline size={18} /></button>

          {/* Notification */}
          <div className="position-relative">
            <button className="btn btn-light rounded-circle shadow-sm p-2"><FiBell size={18} /></button>
          ...
          <div className="dropdown">
            ...
            <ul className="dropdown-menu dropdown-menu-end shadow border-0 rounded-3 p-0 overflow-hidden" aria-labelledby="dropdownUser" style={{ minWidth: "220px" }}>
              ...
              <li>
                <Link className="dropdown-item d-flex align-items-center gap-2 py-2" href="/doctor/account">
      ...
      <div className="d-flex flex-grow-1">
        {/* Sidebar */}
        <aside
          className="d-none d-md-flex flex-column p-3 shadow-sm"
          style={{ width: 230, background: "linear-gradient(180deg, #1e293b, #0f172a)", color: "#f1f5f9" }}
        >
          <nav>
            <ul className="nav nav-pills flex-column gap-1">
              <li>
                <Link href="/doctor" className="nav-link d-flex align-items-center gap-2 sidebar-link active">
                  <MdDashboard size={18} className="text-info" /> Dashboard
                </Link>
              </li>
              <li>
                <Link href="/doctor/item1" className="nav-link d-flex align-items-center gap-2 sidebar-link">
        ...
        {/* Content */}
        <main className="flex-grow-1 p-3 p-md-4 bg-light">{children}</main>
      </div>
    </div>
  );
}
